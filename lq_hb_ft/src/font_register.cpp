...
void lq_hb_ft_font_source_pool_destroy(lq_hb_ft_font_source_pool_t* sources)
{
	...
	for (lq_uint32_t i = 0; i < sources->count; i++)
	{
		lq_hb_ft_font_source_t* source = lq_hb_ft_font_source_pool_get(sources, i);
		LQ_DEBUG_ONLY(lq_utf8_str_destroy(source->name));
		lq_utf8_str_destroy(source->path);
	}
...
void lq_hb_ft_font_family_pool_destroy(lq_hb_ft_font_family_pool_t* families)
{
	...
	for (lq_uint32_t i = 0; i < families->count; i++)
	{
		lq_hb_ft_font_family_t* family = lq_hb_ft_font_family_pool_get(families, i);
		lq_hb_ft_font_source_pool_destroy(&family->sources);
		lq_utf8_str_destroy(family->name);
	}
...
{
	LQ_DEBUG_ASSERT(families != NULL, "families must not be NULL");
	LQ_DEBUG_ASSERT(index < families->count, "index out of bounds");
	return (lq_hb_ft_font_family_t*)lq_array_get(families->array, index);
}
...
void lq_hb_ft_font_instance_pool_destroy(lq_hb_ft_font_instance_pool_t* instances)
{
	...
	for (lq_uint32_t i = 0; i < instances->count; i++)
	{
		lq_hb_ft_font_instance_t* instance = lq_hb_ft_font_instance_pool_get(instances, i);
		lq_hb_ft_font_deinit(&instance->font);
		lq_font_query_destroy(&instance->query);
	}
...
{
	LQ_DEBUG_ASSERT(instances != NULL, "instances must not be NULL");
	LQ_DEBUG_ASSERT(index < instances->count, "index out of bounds");
	return (lq_hb_ft_font_instance_t*)lq_array_get(instances->array, index);
}
...
lq_core_font_interface_t lq_hb_ft_font_register_find_or_create(lq_hb_ft_font_register_t font_register, const lq_font_query_t* query)
{
	...
	for (lq_uint32_t i = 0; i < font_family->sources.count; i++)
	{
		...
		*candidate_source = lq_hb_ft_font_source_create_copy(source);
	}
	...
	for (lq_uint32_t i = 0; i < candidate_sources_ptr->count; i++)
	{
		...
		if (candidate_source->weight == query->weight)
		{
			...
			new_instance->query = lq_font_query_create_copy(query);
